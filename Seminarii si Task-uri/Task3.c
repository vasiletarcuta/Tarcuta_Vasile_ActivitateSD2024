...
//}
//
//struct Televizor* citireFisier(const char* numeFisier, int* nrTV) {
//    FILE* file = fopen(numeFisier, "r");
//    char buffer[100];
//    char delimitare[] = ",\n ";
//    struct Televizor* Televizoare = NULL;
//    (*nrTV) = 0;
//    while (fgets(buffer, 100, file) != NULL) {
//        char* token = strtok(buffer, delimitare);
//        struct Televizor TV;
//        TV.cod = atoi(token);
//        token = strtok(NULL, delimitare);
//        TV.marca = (char*)malloc(strlen(token) + 1);
//        strcpy(TV.marca, token);
//        token = strtok(NULL, delimitare);
//        TV.diagonala = atof(token);
//        token = strtok(NULL, delimitare);
//        TV.rezolutie[0] = atoi(token);
//        token = strtok(NULL, delimitare);
//        TV.rezolutie[1] = atoi(token);
//        token = strtok(NULL, delimitare);
//        TV.pret = atof(token);
//    }
//    fclose(file);
...
//    //task 3
//
//    struct Televizor* TV1 = NULL;
//    int nrTV = 0;
//    TV1 = citireFisier("vectorTVInTxt.txt", &nrTV);
//    for (int i = 0; i < nrTV; i++) {
//        printf("Televizorul are id-ul %d, marca %s, diagonala de %5.2f, rezolutia de %d x %d si pretul %5.2f\n", TV1[i].cod, TV1[i].marca, TV1[i].diagonala, TV1[i].rezolutie[0], TV1[i].rezolutie[1], TV1[i].pret);
//    }
//
